...
当前服务端 key 上限仍为 512KiB，value 上限为 5MiB；Java 容量压测逐条随机生成 1MiB–5MiB 的 value，1MiB 只是压测下限，服务端仍接受更小的 value。单请求总预算保持 8MiB，因此批量 MSET 仍受总大小限制。更新源码后必须重新编译服务端；压测脚本会编译 Java 客户端，已运行的旧测试不会自动切换分布。与旧版 64KiB–1MiB 结果比较时应标注数据大小变化。

35MiB/s 是字节限速，并不保证大 value 的 100ms 延迟：5MiB 写入仅限速预算就约需 143ms，EXPIRE 的保守预算也随最大 value 增大。低速配置还需覆盖单次写入的预算，否则请求可能被限速器拒绝。
